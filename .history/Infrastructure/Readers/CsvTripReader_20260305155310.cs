        ...
        public async Task<(List<Trip> data, List<Trip> duplicates)> ReadAllAsync(string source)
        {
            ...
            await foreach(var r in records)
            {
                ...
                var trip = new Trip
                {
                    ...
                    FareAmount = decimal.Parse(i["fare_amount"]?.ToString() ?? "0", CultureInfo.InvariantCulture),
                    TipAmount = decimal.Parse(i["tip_amount"]?.ToString() ?? "0", CultureInfo.InvariantCulture)
                };
