    ...
    public class CsvTripReader : ICsvReader
    {
        public async Task<(List<Trip> data, List<Trip> duplicates)> ReadAllAsync(string source)
        {
            ...
            await foreach(var r in records)
            {
                ...
                // Використовуємо CultureInfo.InvariantCulture для дат теж
                ...
                var trip = new Trip
                {
                    ...
                    DropoffTime = dropOffUtc, // Зверни увагу: DropoffTime (без великої O, якщо так у моделі)
        ...
        // Цей метод обов'язковий, щоб GetValue працював!
